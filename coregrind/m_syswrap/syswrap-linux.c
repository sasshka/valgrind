                           ...
                           Int fd, Addr vector, Int count, const char *str)
{
   ...
   if (!ML_(fd_allowed)(fd, str, tid, False)) {
      ...
      if (ML_(safe_to_deref) ((void *)(Addr)vector, count * sizeof(struct vki_iovec))) {
          if (vector != 0) {
...
PRE(sys_pwritev2)
{
   *flags |= SfMayBlock;
   ...
   PRE_REG_READ6(ssize_t, "pwritev2",
                 ...
                 vki_u32, offset_high, unsigned long, flags);
#elif VG_WORDSIZE == 8
   PRINT("sys_pwritev2 ( %lu, %#lx, %lu, %ld, %lu )", ARG1, ARG2, ARG3, SARG4, ARG5);
   PRE_REG_READ5(ssize_t, "pwritev2",
                 unsigned long, fd, const struct iovec *, vector,
                 unsigned long, count, Word, offset, unsigned long, flags);
#else
#  error Unexpected word size
